...
# Date: 18-10-4
#讲解链接：https://blog.csdn.net/vivian_ll/article/details/89840281
import os
import ahocorasick
    ...
    def classify(self, question):
        ...
        if not medical_dict:
            if "diseases_dict" in globals():
                medical_dict=diseases_dict# 判断是否是首次提问，若首次提问，则diseases_dict无值
            else:
                return {}
        data['args'] = medical_dict
        #收集query问句当中所涉及到的实体类型,结合特征词对问句进行分类
        types = []
        ...
        if self.check_words(self.food_qwds+self.cure_qwds, question) and 'food' in types:
            deny_status = self.check_words(self.deny_words, question)
            if deny_status:
                question_type = 'food_not_disease'#忌吃
            else:
                question_type = 'food_do_disease'#宜吃
            question_types.append(question_type)
